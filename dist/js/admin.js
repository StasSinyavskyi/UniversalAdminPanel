 var statusLogin=false;//true-login, false-not
        ...
        function loginAjax(){
            ...
            serial=new FormData(document.forms.ajaxForm);
           
            ajax_post('http://catalog/api/login/', serial, function(data) {
        ...
        function dashboardAjax(){
            serial="";
            ajax_get('http://catalog/api/', serial, function(data) {
                if (!checkForErrors(data)) {
                    showDashboardMenue();
        ...
        function getEntityAjax(entity, page='1'){
            ...
            ajax_get('http://catalog/api/'+entity+pageString, serial, function(data) {
                if (!checkForErrors(data)) {
                    showDashboardMenue();
        ...
        function getEntityDataForEditAjax(entity,entityID){
            serial="";
            ajax_get('http://catalog/api/'+entity+'/'+entityID, serial, function(data) {
                if (!checkForErrors(data)) {
                    showDashboardMenue();
                    changeStatusLine(globalSettings[entity].title+" ID: "+entityID);
        ...
        function getEntityDataForAddAjax(entity){
            serial="";
            ajax_get('http://catalog/api/'+entity+'/0',serial, function(data) {
                if (!checkForErrors(data)) {
                    showDashboardMenue();
        ...
        function  entityEditAjax(entity,entityID){
            ...
            ajax_put('http://catalog/api/'+entity+'/'+entityID, serial, function(data) {
                if (!checkForErrors(data)) {
        ...
        function  entityDeleteAjax(entity,entityID){
            ...
            ajax_delete('http://catalog/api/'+entity+'/'+entityID, entity, entityID, function(data) {
                if (!checkForErrors(data)) {
        ...
        function changeUserLoginStatus(){
            ...
            //}
                ...
                id('header-email').setAttribute("data-title",window.sessionStorage['useremail']);
        ...
        function loadPagination(data,page,entity){
            ...
            cont+='<li '+((position > 1)?'class="previous" > <span onclick="Hash.set'+"('"+entity+'/'+(page-1)+"')"+'"> « Previous </span>':'class="previous-off" > « Previous ');
            cont+='</li>';
            
            z=1;
            for (var i = firstPageNumber; i <= firstPageNumber+currentPageCount-1; i++) {
                cont+='<li '+((z==position)?' class="active">'+i:' class="page-number"><span onclick="Hash.set'+"('"+entity+'/'+i+"')"+'">'+i+'</span>');
                cont+='</li>';    
                z++;
            }
            
            
            cont+='<li '+((position < pagesOnPage)?'class="next"> <span onclick="Hash.set'+"('"+entity+'/'+(+page+1)+"')"+'"> next » </span>':'class="next-off" > next »');
            cont+='</ul></div>';
            cont+='</li>';
        ...
        function showDashboardMenue(){
            ...
            for (var i in globalSettings) {
                ...
                dl+='<li onclick="Hash.set('+"'"+i+"'"+');">'+ globalSettings[i].title+'</li>';   
            }
        ...
        function getEntityContent(entityName, data, isDash=false){
            ...
            cont += '<div class="table-title "><span>'+globalSettings[entityName].title+'</span> '+(getButton("Add new","Hash.set('"+entityName+"/add');","add"))+(getButton("Filter","showFilter('"+'filter-'+entityName+"',this);","filter"))+'</div>';
            ...
            for(var settingsItem in globalSettings[entityName].elements){                                      
                ...
                if (settingsString['type']=="n"){   
                    if (settingsString['filterExclude']=="1"){continue;}
                    
                    filterItemContent+='<td><span>'+settingsString['title']+' <u>From:</u></span></td><td><input type="number" name="filter-'+entityName+'-'+settingsString['name']+'-from'+'" /></td> <td><span> <u>To:</u></span></td><td><input type="number" name="filter-'+entityName+'-'+settingsString['name']+'-to'+'" /></td>'; 
                                     
                    filterContent += getInTr('filter-item-wrap','',filterItemContent);                 
                }else if (settingsString['type']=="d"){
                    if (settingsString['filterExclude']=="1"){continue;}
                    filterItemContent+= '<td><span>'+settingsString['title']+': </span></td>';               
                    filterItemContent+='<td><select name="filter-'+entityName+'-'+settingsString.name+'" size="1"> '; 
            
                    // entering Options 
                    var optionString= entityData[0].attachdata[settingsString.name];
                    for(var option in optionString){
                        filterItemContent+='<option value="'+optionString[option].id_category+'">'+optionString[option].title+"</option>";
                    }
                    ...
                    for(var radio in radioString){
                        filterItemContent+='<option value="'+radioString[radio].keyfield+'">'+radioString[radio].name+"</option>";                      
                    }
                    filterItemContent+='</select></td>'; 
            ...
            //submit button
            filterContent +='<td></td><td>'+(getButton("Submit","filterApply('"+entityName+"');","submit"))+'</td>';
            ...
            for (var j in entity){
                if (entity[j].showInGrid=="1"){
                    cont +='<th class="draggable" width="'+entity[j].gridWidth+'"><span>'+ entity[j].title+'</span></th>';  
            ...
            for (var j in entity){
                if (entity[j].showInGrid=="1"){
                    cont +='<th class="button-title" width="'+entity[j].gridWidth+'"><span>'+ entity[j].title+'</span></th>';  
            ...
            for (var row in entityData){
                ...
                keyNumber="";
                    ...
                    for(var settingsItem in globalSettings[entityName].elements){
                        ...
                        if (settingsString['isKey']=="1"){keyNumber=entityData[row][settingsString.name]}
                    

                        if (settingsString.showInGrid=="0"){continue;}
                        ...
                        if (settingsString.parentDependant=="2"){
                            for (var itm in settingsString.items){
                                ...
                                if (settingsString.items[itm].keyfield==entityData[row][settingsString.name]){
                    ...
                    for(var settingsButton in globalSettings[entityName].buttons){
                        if (globalSettings[entityName].buttons[settingsButton].showInGrid=="0"){continue;}
                            actionString="";
                            switch (globalSettings[entityName].buttons[settingsButton].action){
                                case "delete":
                                    actionString="itemDelete('"+entityName+"','"+keyNumber+"')";
                                    break;
                                case "edit":  
                                    actionString="itemEdit('"+entityName+"','"+keyNumber+"')";
                                    break;
                                case "close": 
                                    actionString="itemClose('"+entityName+"','"+keyNumber+"')";
                                    break;    
                                }
                            
                            cont +='<td class="button"><span onclick="'+actionString+'">'+ globalSettings[entityName].buttons[settingsButton].title+'</span></td>';
                    }
        ...
        function showEntityEditAddForm(entity,entityID,data){
            var cont='<div class="edit-title"> Entity edit <span id="edit-title-not-saved"> * </span></div> ';
            var entityData =data[entity];
            
            
            numberTextareasFoReplaceToCkEditor=0;
            
            cont+=' <form action="#" name="entityEditAjaxForm" id="edit-form" method="put" entity="'+entity+'" entityID="'+entityID+'" enctype="multipart/form-data" onchange="changeStatusEditFormNotSavedToOn();" > ';
            
            //add two system inputs
            cont+='<input type="text" name="_id" value="'+entityID+'" style="display:none">';
            cont+='<input type="text" name="_entity" value="'+entity+'" style="display:none">';
             
            for(var settingsItem in globalSettings[entity].elements){
                settingsString=globalSettings[entity].elements[settingsItem]; 
                try
                    {cont += window['getEntityElementType_'+settingsString.type](settingsString,entityData);}
                catch (err) {//alert("probably unsupported entity element Type "+settingsString.type);
                            }                                   
            }
                
            cont+='<input type="submit" name="submit" id="edit-form-submit-button" value="Save data"  />';
            ...
            fm.addEventListener('submit', function(e) {
                ...
                entityEditAjax(e.target.getAttribute("entity"),e.target.getAttribute("entityID"));
        ...
        function getFormatedEntityElement(title,value){
              return '<div class="edit-item-container"><div class="edit-item-title">'+ title +'</div> <div class="edit-item-content">'+ value+' </div></div>';                      
        }
        ...
        function getEntityElementType_s(settingsString,entityData){       
            ...
            title=settingsString.title;  //<input type="password" id="login-form-pass" name="pass" value="827ccb0eea8a706c4c34a16891f84e7b">
            value='<input type="text" name="'+settingsString.name+'" value="'+entityData[0][settingsString.name]+'" '+((settingsString.read==1)?'disabled':'')+'>'; 
         ...
         function getEntityElementType_n(settingsString,entityData){       
            ...
            title=settingsString.title;  //<input type="password" id="login-form-pass" name="pass" value="827ccb0eea8a706c4c34a16891f84e7b">
            value='<input type="number" name="'+settingsString.name+'" value="'+entityData[0][settingsString.name]+'" '+((settingsString.read==1)?'disabled':'')+'>'; 
            return getFormatedEntityElement(title,value);
        }          
         ...
         function getEntityElementType_d(settingsString,entityData){
            ...
            value='<select name="'+settingsString.name+'" size="1" '+((settingsString.read==1)?'disabled':'')+'> '; 
            ...
            if (settingsString.read==1){
                value+='<option value="'+checkedOption+'" selected>'+entityData[0][settingsString.name+'par']+"</option>";
            } else{
                for(var option in optionString){
                    value+='<option value="'+optionString[option].id_category+'" '+((checkedOption==optionString[option].id_category)?'selected':'')+'>'+optionString[option].title+"</option>";
                }
            } 
            value+='</select>';
            return getFormatedEntityElement(title,value); 
        ...
        function getEntityElementType_t(settingsString,entityData){
            ...
            title=settingsString.title; 
            value='<textarea  name="'+settingsString.name+'" id="textareaForEditor'+numberTextareasFoReplaceToCkEditor+'" '+((settingsString.read==1)?'disabled':'')+'>'+entityData[0][settingsString.name]+'</textarea> ';  
        ...
        function getEntityElementType_rb(settingsString,entityData){
            ...
            radioString= settingsString.items;
            for(var radio in radioString){
                value+='<input type="radio" name="'+settingsString.name+'" '+((settingsString.read==1)?'disabled':'')+' value="'+radioString[radio].keyfield+'"'+((checkedRadio==radioString[radio].keyfield)?'checked':'')+'>'+radioString[radio].name+"<br>";
            }
        ...
        function getEntityElementType_is(settingsString,entityData){                 
            ...
            for(var image in imagesList){
                name=settingsString.name+'_'+imagesList[image][settingsString.attachment.id];
            ...
            numberNewImages=1;
            value+=getFormEditOneImageTempl(settingsString.name+'_new_'+numberNewImages, '', read, numberNewImages, settingsString.name);
        ...
        function getFormEditOneImageTempl(name, imgSrc, read, newImg='', entityName=''){
            var value="";
            
            value+= '<div class="edit-item-img-container" id="mark-'+name+'"><span id="edit-item-img-title-'+name+'">'+imgSrc+'</span><br><div id="edit-item-img-'+name+'"><img src="/uploads/'+imgSrc+'" class="edit-item-img" width=200 ></div><br>'; 
           
            value+='<input type="file" name="'+name+'" value="111" id="'+name+'" '+((read==1)?'disabled':'')+' newImg="'+newImg+'" entityName="'+entityName+'" onchange="handleFileSelect(this);"><br><br>'; 
            value+= (getButton("Clear","clearImgFormInput('"+name+"','"+newImg+"');","clear"));         
            value+='</div>';
         ...
         function getEntityElementType_vs(settingsString,entityData){                 
            ...
            for(var video in videoList){
                name=settingsString.name+'_'+videoList[video][settingsString.attachment.id];
            ...
            //add empty dif foe adding new image
            numberNewVideos=1;
            value+=getFormEditOnevideoTempl(settingsString.name+'_new_'+numberNewVideos, '', read, numberNewVideos, settingsString.name);
         ...
         function getFormEditOnevideoTempl(name, videoSrc, read, newvideo='', entityName=''){
            var value="";
            
            value+= '<div class="edit-item-img-container" id="mark-'+name+'"><span id="edit-item-img-title-'+name+'">'+videoSrc+'</span><br><div id="edit-item-img-'+name+'"><video src="/uploads/'+videoSrc+'" class="edit-item-img" width=200 controls="controls" </video> </div><br>'; 
          
            value+='<input type="file" name="'+name+'" id="'+name+'" '+((read==1)?'disabled':'')+' newImg="'+newvideo+'" entityName="'+entityName+'" onchange="handleVideoSelect(this);"><br><br>'; 
            value+= (getButton("Clear","clearVideoFormInput('"+name+"','"+newvideo+"');","clear"));         
            value+='</div>';
        ...
        function clearImgFormInput(objName,newNumber){
            if (!confirm('Do you want to clear uploaded image?')){return;}
        ...
        function clearVideoFormInput(objName,newNumber){
            if (!confirm('Do you want to clear uploaded video?')){return;}
        ...
        function handleFileSelect(evt) {
            ...
            if (evt.getAttribute("newImg")!=""){
                ...
                if (evt.getAttribute("newImg")==numberNewImages){
                    ...
                    value = getFormEditOneImageTempl(entityName+'_new_'+numberNewImages, '', '0', numberNewImages, entityName);
            ...
            for (var i = 0, f; f = files[i]; i++) {
              ...
              reader.onload = (function(theFile) {
                return function(e) {
                  // Render thumbnail.
                  var span = document.createElement('span');
                  span.innerHTML = ['<video class="thumb" controls="controls"  width=200 src="', e.target.result,
                                    '" title="', escape(theFile.name), '"/>'].join('');
        ...
        function itemDelete(entityName,id){
             ...
             if (!confirm('Do you want to delete item ID:'+id+' from '+entityName+'?')){return;}
        ...
        function getInDiv(className,idName,content){
         ...
         function getInTr(className,idName,content){
            return '<tr class="'+className+'" id="'+idName+'">'+content+'</tr>';
        }
        ...
        function getButton(title,url,type){
            if (type=="add"){
                ...
                return '<div class="button-div" onclick="'+url+'">'+title+'</div>';
            } else if (type=="filter"){
                return '<button onclick="'+url+'">'+title+'</button>'; 
            } else if (type=="submit"){
                return '<div class="button-div" onclick="'+url+'">'+title+'</div>';    
        ...
        function ajax_get(url, params, callback) {
            ...
            xmlhttp.setRequestHeader('usl', getUsl());
            xmlhttp.setRequestHeader('usp', getUsp());
            xmlhttp.setRequestHeader('Access-Control-Allow-Headers', 'Access-Control-Allow-Origin, Access-Control-Allow-Headers, usl, usp, rowcount, rowpp, ajax, Origin,Accept, X-Requested-With, Content-Type, Access-Control-Request-Method, Access-Control-Allow-Methods, Access-Control-Request-Headers');
        ...
        function ajax_put(url, params, callback) {
            ...
            xmlhttp.setRequestHeader('Access-Control-Allow-Headers', 'Access-Control-Allow-Origin, Access-Control-Allow-Headers, usl, usp, Origin,Accept, X-Requested-With, Content-Type, Access-Control-Request-Method, Access-Control-Allow-Methods, Access-Control-Request-Headers');
        ...
        function ajax_delete(url, entity, entityId, callback) {
            ...
            xmlhttp.setRequestHeader('Access-Control-Allow-Headers', 'Access-Control-Allow-Origin, Access-Control-Allow-Headers, usl, usp, entity, entityid, Origin,Accept, X-Requested-With, Content-Type, Access-Control-Request-Method, Access-Control-Allow-Methods, Access-Control-Request-Headers');
     ...
     window.onhashchange= function(){hashChange();}
        ...
        var Hash = {
            ...
            get: function() {
                ...
                else {vars = hashes.split(splitter);}            
                return vars;
